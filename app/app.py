...
def login():
    ...
    if (user_df[user_df['id'] == user_id]['Name'] == user_name).any():
        print("1st")
        return jsonify({"message": "Login Successful", "success":True, "name":user_name, "id":user_id})
    else:
        print("3rd")
...
def register_user():
    ...
    return jsonify({"message": "User registered successfully", "id": user_id, "name": user_name, "success": True}), 201
